...
class JoyToAckNode(Node):
    ...
    def lidar_callback(self,msg):
        ...
        pt1x = d1*math.cos(theta * beam_angle)
        pt2x = d2*math.cos(theta * beam_angle)
        
        phi = math.asin((d1/d3)*math.sin(theta * beam_angle))
        #dw = d1 * math.cos(90 - theta - phi)
        dw = d1 * math.cos((pt2x - pt1x)/d3)

        #Crane's equation for comparison
        #print(f'Crane\'s equation: {d1 * math.cos((pt2x - pt1x)/d3)}')
        ...
        msg_send.drive.steering_angle = -(math.pi/12 * self.uk) 
